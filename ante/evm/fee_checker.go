...
func NewDynamicFeeChecker(fmKeeper anteinterfaces.FeeMarketKeeper, cpKeeper anteinterfaces.ControlPanelKeeper) authante.TxFeeChecker {
	return func(ctx sdk.Context, tx sdk.Tx) (sdk.Coins, int64, error) {
		...
		return FeeChecker(ctx, fmKeeper, cpKeeper, denom, ethCfg, feeTx)
	}
}
...
func FeeChecker(
	ctx sdk.Context,
	fmKeeper anteinterfaces.FeeMarketKeeper,
	...
	isSponsored := cpKeeper != nil && cpKeeper.IsSponsoredAddress(ctx, feeTx.FeePayer())

	if !isSponsored && feeCap.LT(baseFee) {
	...
	if isSponsored {
		priority = cpKeeper.GetSponsoredTransactionPriority(ctx)
	}
